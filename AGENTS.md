...
SaaS database naming is part of the project contract:

- The SaaS platform management database is `meta_org_saas`.
- Physical tenant business databases are named `meta_org_xxxx`, where `xxxx` is the first four lowercase hexadecimal characters of the tenant organization UUID with hyphens removed.
- `TENANT_DATABASE_NAME_PREFIX` must default to `meta_org_`; tenant database names must be generated by the backend tenant database helper, not by ad hoc string concatenation in handlers or SQL outside migrations.
- `TENANT_DATABASE_ADMIN_URL` must point at an administrative database such as `postgres` on the same PostgreSQL instance, not at the platform database itself.
- Fresh SaaS setup must create and migrate `meta_org_saas` first, then create tenant databases through the tenant database provisioner so `tenantdb:include` directives are expanded and tenant ERP/finance tables are present.
- Do not restore the old single database `meta_org` as the active runtime database. Treat it only as an explicitly named backup or migration source.
