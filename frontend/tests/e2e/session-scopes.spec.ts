...
test('tenant session is explicitly scoped', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/tenant\/[0-9a-f-]+\/overview$/)
  await expect(page.getByTestId('session-scope')).toBeVisible()
  await expect(page.getByTestId('login-shell')).toHaveCount(0)
  await openNavigationIfNeeded(page)
  await page.getByTestId('domain-nav-Project').click()
  await expect(page).toHaveURL(/\/tenant\/[0-9a-f-]+\/projects$/)
  ...
  await expect(page.getByTestId('session-scope')).toContainText('Tenant operations')
  await expect(page.getByTestId('domain-nav-FinanceAccounting')).toContainText('Finance Accounting')
  await expect(page.locator('body')).not.toContainText('scope.tenant')
  await expect(page.locator('body')).not.toContainText(/Finance(Accounting|Receivables|Payables|CostAccounting)/)
  await page.getByRole('button', { name: '中文', exact: true }).click()
  await expect(page.getByTestId('session-scope')).toContainText('租户业务台')
  await expectNoHorizontalOverflow(page)
})
