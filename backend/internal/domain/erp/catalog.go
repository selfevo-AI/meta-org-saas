...
func DefaultCatalog() Catalog {
	tables := []TableDefinition{
		...
		table("MBOM", "Bill of Materials", "manufacturing", "BOMCode", []FieldDefinition{
			field("BOMCode", "nVarChar", "64", "BOM Code", true),
			field("Name", "nVarChar", "155", "BOM Name", false),
			field("Status", "VarChar", "20", "Status", false),
			field("ItemCode", "nVarChar", "50", "Finished Item Code", false),
			field("Quantity", "Numeric", "19,6", "Finished Quantity", false),
			field("SourceWhsCode", "nVarChar", "8", "Source Warehouse", false),
			field("WipWhsCode", "nVarChar", "8", "WIP Warehouse", false),
			field("FinishedWhsCode", "nVarChar", "8", "Finished Goods Warehouse", false),
			field("ApprovedBy", "nVarChar", "64", "Approved By", false),
		}, child("BOM1", "Bill of Materials - Components", "BOMCode")),
		table("MWOR", "Work Order", "manufacturing", "WorkOrderCode", []FieldDefinition{
			field("WorkOrderCode", "nVarChar", "64", "Work Order Code", true),
			field("Name", "nVarChar", "155", "Work Order Name", false),
			field("Status", "VarChar", "20", "Status", false),
			field("BOMCode", "nVarChar", "64", "BOM Code", false),
			field("ItemCode", "nVarChar", "50", "Production Item Code", false),
			field("Quantity", "Numeric", "19,6", "Production Quantity", false),
			field("ProducedQuantity", "Numeric", "19,6", "Produced Quantity", false),
			field("SourceWhsCode", "nVarChar", "8", "Source Warehouse", false),
			field("WipWhsCode", "nVarChar", "8", "WIP Warehouse", false),
			field("FinishedWhsCode", "nVarChar", "8", "Finished Goods Warehouse", false),
			field("MaterialIssued", "VarChar", "1", "Material Issued", false),
			field("Posted", "VarChar", "1", "Posted", false),
		}, child("WOR1", "Work Order - Required Items", "WorkOrderCode")),
		...
		extensionTable("MBRN", "Stores", "retail", "StoreCode", "BRN1", "Store Rows"),
		extensionTable("MTER", "POS Terminals", "retail", "TerminalCode", "TER1", "POS Terminal Rows"),
		extensionTable("MMBR", "Members", "retail", "MemberCode", "MBR1", "Member Rows"),
		extensionTable("MPRM", "Promotions", "retail", "PromotionCode", "PRM1", "Promotion Rows"),
		extensionTable("MPUB", "Item Publications", "retail", "PublicationCode", "PUB1", "Item Publication Rows"),
		documentTable("MRPS", "POS Sale", "retail", "RPS1"),
		documentTable("MDRQ", "Distribution Request", "retail", "DRQ1"),
		documentTable("MDSP", "Distribution Shipment", "retail", "DSP1"),
		documentTable("MDRC", "Distribution Receipt", "retail", "DRC1"),
		documentTable("MDIF", "Distribution Difference", "retail", "DIF1"),
		extensionTable("MSTP", "Stock Policy", "retail", "PolicyCode", "STP1", "Stock Policy Rows"),
		documentTable("MCNT", "Store Count", "retail", "CNT1"),
		documentTable("MSPR", "Special Purchase Request", "retail", "SPR1"),
...
func defaultModules() []ModuleDefinition {
	return []ModuleDefinition{
		...
		moduleDef("manufacturing", "Manufacturing",
			submoduleDef("bom", "BOM", docDef("bill_of_materials", "Bill of Materials", "MBOM", []string{"BOM1"}, []string{"approve", "make-work-order"})),
			submoduleDef("work_orders", "Work Orders", docDef("work_order", "Work Order", "MWOR", []string{"WOR1"}, []string{"release", "issue-material", "complete", "close", "stop", "reopen"})),
		),
		...
		moduleDef("retail", "Retail",
			submoduleDef("stores", "Stores", docDef("store", "Store", "MBRN", []string{"BRN1"}, nil)),
			submoduleDef("pos", "POS", docDef("pos_terminal", "POS Terminal", "MTER", []string{"TER1"}, nil), docDef("pos_sale", "POS Sale", "MRPS", []string{"RPS1"}, []string{"close"})),
			submoduleDef("members_promotions", "Members and Promotions", docDef("member", "Member", "MMBR", []string{"MBR1"}, nil), docDef("promotion", "Promotion", "MPRM", []string{"PRM1"}, nil), docDef("item_publication", "Item Publication", "MPUB", []string{"PUB1"}, []string{"publish"})),
			submoduleDef("distribution", "Distribution", docDef("distribution_request", "Distribution Request", "MDRQ", []string{"DRQ1"}, []string{"submit", "approve", "auto-allocate"}), docDef("distribution_shipment", "Distribution Shipment", "MDSP", []string{"DSP1"}, []string{"ship"}), docDef("distribution_receipt", "Distribution Receipt", "MDRC", []string{"DRC1"}, []string{"receive"}), docDef("distribution_difference", "Distribution Difference", "MDIF", []string{"DIF1"}, []string{"resolve"})),
			submoduleDef("inventory_control", "Inventory Control", docDef("stock_policy", "Stock Policy", "MSTP", []string{"STP1"}, []string{"replenish"}), docDef("store_count", "Store Count", "MCNT", []string{"CNT1"}, []string{"submit", "approve", "post-adjustment"})),
			submoduleDef("special_procurement", "Special Procurement", docDef("special_purchase_request", "Special Purchase Request", "MSPR", []string{"SPR1"}, []string{"submit", "approve", "convert-to-purchase-order"})),
		),
