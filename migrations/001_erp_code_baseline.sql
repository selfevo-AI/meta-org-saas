...
-- The lifecycle domain owns project data. MPRJ/APRJ remain ERP-compatible
-- names, but are projections instead of a second project store.
...
BEGIN
    IF TO_REGCLASS('"MREQ_legacy"') IS NOT NULL THEN
        ...
        SELECT COALESCE(erp_try_uuid(m."ReqCode"), GEN_RANDOM_UUID()),
               ...
               COALESCE(NULLIF(m."Payload"->>'Source', ''), 'erp'),
...
SELECT COALESCE(NULLIF(r.metadata->>'erp_requirement_code', ''), r.id::TEXT) AS "ReqCode",
       (r.metadata - 'erp_requirement_code') || JSONB_BUILD_OBJECT(
           'ReqCode', COALESCE(NULLIF(r.metadata->>'erp_requirement_code', ''), r.id::TEXT),
           'RequirementID', r.id::TEXT, 'RequirementCode', r.id::TEXT,
           'Name', r.title, 'Title', r.title, 'Description', r.description, 'Source', r.source,
...
BEGIN
    ...
    UPDATE requirements r SET organization_id = COALESCE(r.organization_id, v_organization_id),
        ...
        budget_amount = COALESCE(erp_try_numeric(v_data->>'BudgetAmount'), r.budget_amount),
        budget_currency = COALESCE(NULLIF(v_data->>'BudgetCurrency', ''), r.budget_currency),
...
SELECT COALESCE(NULLIF(r.metadata->>'erp_requirement_code', ''), r.id::TEXT) AS "ReqCode",
       ROW_NUMBER() OVER (PARTITION BY d.requirement_id ORDER BY d.created_at, d.id)::BIGINT AS "LineNum",
       'O'::VARCHAR(1) AS "LineStatus",
...
BEGIN
    ...
    IF v_kind = 'r' AND TO_REGCLASS('"MPRJ_legacy"') IS NULL THEN
        ALTER TABLE "MPRJ" RENAME TO "MPRJ_legacy";
    END IF;
    SELECT relkind INTO v_kind FROM pg_class WHERE oid = TO_REGCLASS('"APRJ"');
    IF v_kind = 'r' AND TO_REGCLASS('"APRJ_legacy"') IS NULL THEN
        ALTER TABLE "APRJ" RENAME TO "APRJ_legacy";
    END IF;
END;
$$;
...
BEGIN
    IF TO_REGCLASS('"MPRJ_legacy"') IS NOT NULL THEN
        ...
        SELECT COALESCE(erp_try_uuid(m."PrjCode"), GEN_RANDOM_UUID()),
               ...
               CASE WHEN m."Payload"->>'RiskLevel' IN ('low','medium','high','critical') THEN m."Payload"->>'RiskLevel' ELSE 'low' END,
               CASE WHEN m."Payload"->>'RequiredLevel' IN ('L1','L2','L3','L4') THEN m."Payload"->>'RequiredLevel' ELSE 'L1' END,
               COALESCE(erp_try_numeric(m."Payload"->>'BudgetAmount'), 0),
               COALESCE(NULLIF(m."Payload"->>'BudgetCurrency', ''), 'CNY'),
        ...
        WHERE NOT EXISTS (
            ...
            WHERE p.id = erp_try_uuid(m."PrjCode") OR p.metadata->>'erp_project_code' = m."PrjCode"
        )
        ON CONFLICT (id) DO NOTHING;
    END IF;
END;
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_projects_erp_project_code
    ON projects((metadata->>'erp_project_code'))
...
SELECT COALESCE(NULLIF(p.metadata->>'erp_project_code', ''), p.id::TEXT) AS "PrjCode",
       (p.metadata - 'erp_project_code') || JSONB_BUILD_OBJECT(
           'PrjCode', COALESCE(NULLIF(p.metadata->>'erp_project_code', ''), p.id::TEXT),
           'ProjectID', p.id::TEXT, 'ProjectCode', p.id::TEXT,
           'Name', p.name, 'Description', p.description, 'Status', p.status,
           'Priority', p.priority, 'RiskLevel', p.risk_level, 'RequiredLevel', p.required_level,
           ...
           'Locked', CASE WHEN p.status IN ('closed','cancelled') THEN 'Y' ELSE 'N' END,
           'DataSource', 'P') AS "Payload",
...
BEGIN
    ...
    IF v_requirement_id IS NULL THEN
        ...
        WHERE metadata->>'erp_requirement_code' = COALESCE(NULLIF(v_data->>'RequirementCode', ''), v_data->>'RequirementID')
        LIMIT 1;
    END IF;
    IF TG_OP = 'INSERT' THEN
    ...
    END IF;
    UPDATE projects p
    SET requirement_id = COALESCE(p.requirement_id, v_requirement_id),
        organization_id = COALESCE(p.organization_id, v_organization_id),
        name = COALESCE(NULLIF(v_data->>'Name', ''), p.name),
        description = COALESCE(v_data->>'Description', p.description), status = v_status,
    ...
    WHERE COALESCE(NULLIF(p.metadata->>'erp_project_code', ''), p.id::TEXT) = OLD."PrjCode"
...
CREATE TRIGGER trg_write_mprj_project_projection
INSTEAD OF INSERT OR UPDATE OR DELETE ON "MPRJ"
