...
Errors under `/api/v1` retain the compatible `error` field and add stable `code` and `request_id` fields. The same request ID is returned in `X-Request-ID`, and `X-Error-Code` carries the stable code. Clients should branch on `code`, correlate logs with `request_id`, and never parse free-form error text. OpenAI-compatible `/v1` routes keep their separate protocol contract; `POST /v1/chat/completions` supports standard SSE chunks and the `[DONE]` terminator for `stream: true`, with access-token authorization, balance reservation, actual-usage settlement, and invocation auditing.
...
The expected state is frontend `3000` and backend `8080` both in `Listen`, frontend HTTP `200`, and backend health reporting `status: ok`, `platform_database.status: ok`, and `security_kernel.status: ok`, plus tenant-identifier-free pool aggregates in `tenant_database_pools`. To stop an old process, first confirm the `OwningProcess` from the port query, then run `Stop-Process -Id <PID> -Force` for one PID at a time.
...
| `AI_GATEWAY_RESERVATION_LEASE_SECONDS` | `60` | Multi-replica recovery lease duration. |
| `AI_GATEWAY_RESERVATION_BATCH_SIZE` | `100` | Maximum reservations recovered per run, capped at `1000`. |
